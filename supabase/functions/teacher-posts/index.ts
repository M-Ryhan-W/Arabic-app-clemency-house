...
serve(async (req) => {
  ...
  try {
    ...
    if (roleError) {
      console.error("teacher-posts roles lookup error:", roleError);
    ...
    if (studentError || !student) {
      console.error("teacher-posts student lookup error:", studentError);
      return jsonResponse({ error: "Student not found" }, 404);
    }

    if (postsError) {
      console.error("teacher-posts posts lookup error:", postsError);
    ...
    if (enrichedPosts.length > 0) {
      ...
      if (correctionsError) {
        console.error("teacher-posts corrections lookup error:", correctionsError);
      }
    ...
    console.error("teacher-posts error:", err);
